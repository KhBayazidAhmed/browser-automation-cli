export const INJECTED_MODALS_SRC = `
  ...
  shadow.getElementById("modal-cancel-btn")?.addEventListener("click", () => { closeModal(); showToast("Extract cancelled", true); });
  ...
  shadow.getElementById("modal-assert-save")?.addEventListener("click", () => {
    const type = shadow.getElementById("modal-assert-type")?.value || "strict";
    const val = shadow.getElementById("modal-assert-val")?.value ?? "";
    if (assertCallback) assertCallback(type, val);
    closeAssertModal();
  });
  ...
  btnStop?.addEventListener("click", (e) => { e.stopPropagation(); emitRecordEvent({ type: "finish" }); });
`;
